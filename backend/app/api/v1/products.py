"""Product API endpoints — creation, discovery, feature map.
...
from backend.app.schemas.feature import FeatureResponse, FeatureMapApproval, FeatureMapResponse
...
@router.post("/{product_id}/discovery", response_model=DiscoveryResponse)
async def submit_discovery(
    product_id: UUID, data: DiscoveryAnswerInput, session: AsyncSession = Depends(get_session)
):
...
@router.post("/{product_id}/features", response_model=list[FeatureResponse])
...
@router.post("/{product_id}/features/approve", response_model=FeatureMapResponse)
async def approve_feature_map(
    product_id: UUID, data: FeatureMapApproval, session: AsyncSession = Depends(get_session)
):
    ...
    return FeatureMapResponse(
        ...
        features=[FeatureResponse.model_validate(f) for f in result["features"]],
